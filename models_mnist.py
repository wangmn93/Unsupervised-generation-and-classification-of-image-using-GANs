...
def decoder(z, reuse=True, name="decoder"):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        return x
def decoder22(z, reuse=True, name="decoder"):
    fc_relu = partial(fc, activation_fn=relu)
    fc_sigmoid = partial(fc, activation_fn=tf.nn.sigmoid)
    with tf.variable_scope(name, reuse=reuse):
        y = fc_relu(z, 2000, scope='layer1')
        y = fc_relu(y, 500, scope='layer2')
        y = fc_relu(y, 500,scope='layer3')
        x = fc_sigmoid(y,3072, scope='output_layer')
...
# selective sampling
def ss_generator2(z, reuse=True, name="generator", training=True, heads=10):
    bn = partial(batch_norm, is_training=training)
    # fc_relu = partial(fc, normalizer_fn=None, activation_fn=relu)
    fc_bn_relu = partial(fc, normalizer_fn=bn, activation_fn=relu, biases_initializer=None)
    with tf.variable_scope(name, reuse=reuse):
        ...
        for _ in range(heads):
            out = tf.sigmoid(fc(y, 100))
            img_sets.append(out)
        return img_sets

def ss_generator3(z, reuse=True, name="generator", training=True, heads=10):
    bn = partial(batch_norm, is_training=training)
    # fc_relu = partial(fc, normalizer_fn=None, activation_fn=relu)
    fc_bn_relu = partial(fc, normalizer_fn=bn, activation_fn=relu, biases_initializer=None)
    with tf.variable_scope(name, reuse=reuse):
...
def ss_discriminator2(x, reuse=True, name = "discriminator"):
    fc_lrelu = partial(fc, normalizer_fn=None, activation_fn=lrelu)
    with tf.variable_scope(name, reuse=reuse):
        y = fc_lrelu(x, 1024)
        y = fc_lrelu(y, 512)
...
def ss_classifier(x,keep_prob, reuse=True, name = "classifier", ):
    fc_lrelu = partial(fc, normalizer_fn=None, activation_fn=lrelu)
    with tf.variable_scope(name, reuse=reuse):
        y = fc_lrelu(x, 1024)
        y = fc_lrelu(y, 512)
        ...
        dropout = tf.layers.dropout(
            inputs=y, rate=1 - keep_prob)
        return fc(dropout, 10)
...
def generator_m2_32X32(z, heads=10, training=True, reuse=True, name="generator", bs = 64):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        net = tf.reshape(net, [bs, 8, 8, 128])
        net = tf.nn.relu(
                bn(deconv2d(net, [bs, 16, 16, 64], 4, 4, 2, 2, name='g_dc3'), is_training=training,
                   scope='g_bn3'))

        img_sets = []
        for i in range(heads):
            out = tf.nn.sigmoid(deconv2d(net, [bs, 32, 32, 3], 4, 4, 2, 2, name='g_dc4_h' + str(i)))
            img_sets.append(out)
...
def cnn_discriminator2(z, out_dim=10, reuse=True, name = "discriminator" , training=True):
    lrelu_1 = partial(ops.leak_relu, leak=0.1)
    conv_lrelu = partial(conv, activation_fn=lrelu_1)
    fc_lrelu = partial(fc, activation_fn=lrelu_1)
    with tf.variable_scope(name, reuse=reuse):
        x = tf.reshape(z,shape=[-1, 28, 28, 1])
        y = conv_lrelu(x, 32, 5, 1)
        y = tf.layers.max_pooling2d(inputs=y, pool_size=[3, 3], strides=2)
        y = conv_lrelu(y, 64, 3, 1)
        y = conv_lrelu(y, 64, 3, 1)
        y = tf.layers.max_pooling2d(inputs=y, pool_size=[3, 3], strides=2)
